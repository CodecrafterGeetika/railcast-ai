...
export function ConfidenceRange({ prediction }: { prediction: ETAPrediction }) {
    ...
    <Card>
      ...
      <CardContent>
        <div className="flex items-baseline justify-between">
          <div>
            <p className="text-xs text-muted-foreground">Our predicted arrival</p>
            <p className="font-mono text-2xl font-semibold text-foreground">{prediction.predictedETA}</p>
          </div>
          <div className="text-right">
            <p className="text-xs text-muted-foreground">Confidence</p>
            <p className="text-lg font-semibold text-rail-accent2">{prediction.confidencePercent}%</p>
          </div>
        </div>
        ...
        <p className="mt-4 rounded-md border border-border bg-secondary/30 p-3 text-xs leading-relaxed text-muted-foreground">
          Prediction interval reflects uncertainty in current operating conditions. Statistical
          validity is not claimed until the real ML model is connected to historical outcome data.
        </p>
      </CardContent>
    </Card>
  );
